...
$group = $_GET['group'];
$cursor = $collection->find(['type' => 'лабораторна', 'groups' => $group]);

// Формирование результатов запроса
$results = [];
foreach ($cursor as $doc) {
  $results[] = "{$doc['date']} — {$doc['subject']} ({$doc['auditorium']})";
}
?>
...
<head>
  <meta charset="UTF-8">
  <title>Лабораторні для групи <?php echo htmlspecialchars($group); ?></title>
  <style>
    body {
...
<body>
  <h1>Лабораторні для групи <?php echo htmlspecialchars($group); ?></h1>
  ...
  <div class="results">
    ...
    <?php if (count($results) > 0): ?>
      ...
      <?php endforeach; ?>
    <?php else: ?>
      <p>Лабораторні для групи <?php echo htmlspecialchars($group); ?> не знайдено.</p>
    <?php endif; ?>
  </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const storageKey = 'group_labs_' + group;
      ...
      try {
        ...
        if (Array.isArray(savedData) && savedData.length > 0) {
          ...
          previousResultsElement.innerHTML = "<p>Немає збережених результатів для цієї групи.</p>";
        }
      } catch (e) {
